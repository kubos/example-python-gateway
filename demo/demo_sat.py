        ...
        try:
            if command.type == "ping":
                ...
                try:
                    ...
                    asyncio.ensure_future(major_tom.fail_command(command_id=command.id,errors=["File failed to download", f"Error: {traceback.format_exc()}"]))
                ...
                try:
                    ...
                    image_date = datetime.datetime.strptime(latest_image["date"], "%Y-%m-%d %H:%M:%S")
                    image_filename = latest_image["image"] + ".png"
                    image_url = "https://epic.gsfc.nasa.gov/archive/natural" + image_date.strftime("/%Y/%m/%d") + "/png/" + image_filename

                    # Get the image itself
                    image_r = requests.get(image_url)
                    if image_r.status_code != 200:
                        raise(RuntimeError(f"File Download Failed. Status code: {image_r.status_code}"))
                ...
                try:
                    ...
                    asyncio.ensure_future(major_tom.fail_command(command_id=command.id,errors=["Downlinked File failed to upload to Major Tom", f"Error: {traceback.format_exc()}"]))

                # Remove file now that it's uploaded so we don't fill the disk.
                os.remove(image_filename)

        except Exception as e:
            asyncio.ensure_future(major_tom.fail_command(command_id=command.id,errors=["Command Failed to Execute. Unknown Error Occurred.", f"Error: {traceback.format_exc()}"]))
